...
const WHEEL_OPTIONS = [
  ...
  "Mutass egy szexi képet"
];

const CARD_DECKS = [
  { id: 'romantic', name: 'Romantikus', color: 'from-pink-500 to-rose-500', icon: Heart, labels: ["Mondd el miért szereted", "Idézz fel egy emléket", "Nézz a szemébe 1 percig"] },
  { id: 'spicy', name: 'Szenvedélyes', color: 'from-orange-500 to-rose-600', icon: Zap, labels: ["Érintsd meg a nyakát", "Csókold meg ahol tetszik", "Suttogj egy vágyadat"] },
  { id: 'deep', name: 'Mély', color: 'from-purple-600 to-indigo-700', icon: Sparkles, labels: ["Mi a közös álmotok?", "Mi az első benyomásod?", "Miben bízol a legjobban?"] }
];

const FlameApp = ({ onClose }: { onClose: () => void }) => {
  const [is18Verified, setIs18Verified] = useState(false);
  const [activeTab, setActiveTab] = useState<'wheel' | 'cards' | 'settings'>('wheel');
  const [isSpinning, setIsSpinning] = useState(false);
  const [rotation, setRotation] = useState(0);
  const [wheelResult, setWheelResult] = useState<string | null>(null);
  
  const [activeDeck, setActiveDeck] = useState<typeof CARD_DECKS[0] | null>(null);
  ...
  const spinWheel = () => {
    ...
    setTimeout(() => {
      ...
      const index = Math.floor((360 - actualRotation) / (360 / WHEEL_OPTIONS.length)) % WHEEL_OPTIONS.length;
      setWheelResult(WHEEL_OPTIONS[index]);
    }, 3000);
  };

  const drawCard = (deck: typeof CARD_DECKS[0]) => {
    setActiveDeck(deck);
    const randomLabel = deck.labels[Math.floor(Math.random() * deck.labels.length)];
            ...
            >
              <div className="relative mb-12">
                ...
                <motion.div 
                   ...
                   className="w-72 h-72 sm:w-80 sm:h-80 rounded-full border-[10px] border-zinc-900 shadow-[0_0_50px_rgba(244,63,94,0.1)] relative overflow-hidden bg-zinc-900"
                >
                  {WHEEL_OPTIONS.map((option, i) => (
                    <div 
                      key={i}
                      className="absolute top-0 left-1/2 origin-bottom h-1/2 p-2 pt-6 text-center"
                      style={{ 
                        width: '40px',
                        marginLeft: '-20px',
                    ...
                    >
                      <span className="text-[7px] font-black uppercase tracking-tighter leading-none block rotate-180" style={{ writingMode: 'vertical-rl' }}>
                        {option}
                      </span>
                    </div>
                  ))}
                  {/* CENTRAL HUB */}
                  <div className="absolute inset-0 m-auto w-12 h-12 bg-black border-4 border-rose-500 rounded-full z-10 flex items-center justify-center shadow-2xl shadow-rose-500/50">
                  ...
                  {WHEEL_OPTIONS.map((_, i) => (
                    <div 
                      key={`div-${i}`}
                      className="absolute top-0 left-1/2 h-1/2 w-0.5 bg-white/5 origin-bottom"
                      style={{ transform: `rotate(${(i * (360 / WHEEL_OPTIONS.length)) + (360 / WHEEL_OPTIONS.length / 2)}deg)` }}
                    />
                  ))}
                </motion.div>
              </div>
                  ...
                  >
                    <div className="relative z-10">
                      <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center mb-4 backdrop-blur-md">
                        <deck.icon size={24} className="text-white" />
                      </div>
                      <h3 className="text-2xl font-black italic">{deck.name}</h3>
                      <p className="text-white/60 text-[10px] font-bold uppercase tracking-widest">{deck.labels.length} feladat érhető el</p>
                    </div>
                    <div className="absolute top-0 right-0 p-6 opacity-20 group-hover:opacity-40 transition-opacity">
                      <deck.icon size={80} strokeWidth={1} />
                    </div>
          ...
          >
            ...
            >
              <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md">
                <activeDeck.icon size={32} className="text-white" />
              </div>
              
              <div className="space-y-4">
                <p className="text-white/60 text-[10px] font-black uppercase tracking-widest">{activeDeck.name} kártya</p>
                <h2 className="text-3xl font-black italic uppercase tracking-tighter leading-tight">{currentCardContent}</h2>
              </div>
              ...
              <div className="absolute top-0 right-0 p-8 opacity-10">
                <activeDeck.icon size={120} strokeWidth={1} />
              </div>
            </motion.div>
          </motion.div>
        )}
